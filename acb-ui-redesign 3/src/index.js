...
function App(){
  ...
  return <div className="app">
    <aside className="side">
      <div className="brand">
        <div className="mark">ACB</div>
        <div><b>Agent Control Bureau</b><span>Know. Govern. Maximize.</span></div>
      </div>
      {[['mission','Agent Field View'],['registry','Agent Registry'],['governance','Admin Loop'],['value','Cost Intelligence'],['ask','Ask Bureau']].map(x=><button key={x[0]} onClick={()=>setPage(x[0])} className={page===x[0]?'active':''}>{x[1]}</button>)}
      <div className="sideMeta">
        <span>Runtime visibility</span>
        <b>Observe. Govern. Resolve.</b>
      </div>
    </aside>
    <main>
      <header>
        <div>
          <div className="jpmc">JPMorgan Chase</div>
          <h1>{title(page)}</h1>
        ...
        <div className="headerStats"><span>{stats.agents||0} agents</span><span>{stats.under_review||0} under review</span><span>${stats.cost||0} cost</span></div>
      </header>
...
function title(p){return {mission:'Agent Field View',registry:'Agent Registry',governance:'Admin In The Loop',value:'Cost Intelligence',ask:'Ask Bureau'}[p]}
function statusClass(s){ if(s==='APPROVED')return 'good'; if(s==='BANNED')return 'bad'; if(s==='UNDER_REVIEW')return 'warn'; return 'muted'; }
function statusLabel(s){ return String(s || 'UNKNOWN').replace('_',' '); }

function Mission({agents,activity,stats,onOpen}){
  const positions=[['10%','18%'],['31%','12%'],['53%','23%'],['77%','16%'],['18%','55%'],['43%','48%'],['68%','58%'],['86%','44%'],['33%','77%'],['61%','82%']];
  return <div className="grid missionGrid">
    ...
    <section className="panel radar"><div className="panelTitle"><b>Agent Field View</b><span>Live runtime presence across LOBs</span></div><div className="field">
      <div className="pulseCore">ACB</div><div className="rings"></div>{agents.map((a,i)=><div key={a.id} onClick={()=>onOpen(a)} className={'agentPill '+statusClass(a.status)} style={{left:positions[i%positions.length][0],top:positions[i%positions.length][1]}}>
...
function Governance({approvals,refresh}){ return <div className="panel"><div className="panelTitle"><b>Approval Queue</b><span>Admin can approve or ban agents</span></div>{approvals.length===0&&<div className="empty">No pending approvals</div>}{approvals.map(a=><div className="approval" key={a.id}><div><span className="changeTag">New runtime change</span><h2>{a.agent_name}</h2><p>{a.lob} · {a.team} · {a.model}</p><b>{a.tool_name||a.type}</b><small>{a.reason}</small></div><div className="actions"><button onClick={()=>post('/approvals/'+a.id+'/approve').then(refresh)}>Approve</button><button className="danger" onClick={()=>post('/approvals/'+a.id+'/ban').then(refresh)}>Ban & Kill PID</button></div></div>)}</div> }

function Value({metrics,agents}){ if(!metrics)return null; const top=metrics.topAgents||[]; const lob=metrics.tokensByLob||[]; const model=metrics.modelUsage||[]; const ranked=[...agents].sort((a,b)=>(b.monthly_tokens||0)-(a.monthly_tokens||0)).slice(0,8); return <div className="valueGrid"><section className="panel insightCard"><span>Live API Metrics</span><b>{metrics?.summary?.optimization_signal || 'Connected'}</b><p>All values on this screen are populated from backend metrics, agent inventory and runtime events.</p></section><section className="panel"><div className="panelTitle"><b>Token Consumption by Agent</b><span>Monthly usage</span></div><ResponsiveContainer width="100%" height={260}><BarChart data={top}><XAxis dataKey="agent_name" hide/><YAxis tick={{fill:'#64748b',fontSize:12}}/><Tooltip contentStyle={{background:'#0f1115',border:'1px solid rgba(255,255,255,.12)',borderRadius:12,color:'#fff'}}/><Bar dataKey="tokens" fill="#3B82F6" radius={[10,10,0,0]}/></BarChart></ResponsiveContainer></section><section className="panel"><div className="panelTitle"><b>Cost by LOB</b><span>Cost control lens</span></div><ResponsiveContainer width="100%" height={260}><BarChart data={lob}><XAxis dataKey="lob" tick={{fill:'#64748b',fontSize:12}}/><YAxis tick={{fill:'#64748b',fontSize:12}}/><Tooltip contentStyle={{background:'#0f1115',border:'1px solid rgba(255,255,255,.12)',borderRadius:12,color:'#fff'}}/><Bar dataKey="cost" fill="#6366F1" radius={[10,10,0,0]}/></BarChart></ResponsiveContainer></section><section className="panel wide"><div className="panelTitle"><b>Model Usage</b><span>Which models are driving consumption</span></div><ResponsiveContainer width="100%" height={260}><AreaChart data={model}><CartesianGrid stroke="#e5e7eb" strokeDasharray="3 3"/><XAxis dataKey="model" tick={{fill:'#64748b',fontSize:12}}/><YAxis tick={{fill:'#64748b',fontSize:12}}/><Tooltip contentStyle={{background:'#0f1115',border:'1px solid rgba(255,255,255,.12)',borderRadius:12,color:'#fff'}}/><Area type="monotone" dataKey="tokens" stroke="#60A5FA" strokeWidth={4} fill="#DBEAFE"/></AreaChart></ResponsiveContainer></section><section className="panel"><div className="panelTitle"><b>Highest Token Agents</b><span>Click in registry for detail</span></div>{ranked.map((a,i)=><div className="rank" key={a.id}><b><span className="rankNo">#{i+1}</span>{a.agent_name}</b><span>{Number(a.monthly_tokens||0).toLocaleString()}</span><small>{a.lob} · {a.team}</small></div>)}</section></div> }

function Ask(){ const [q,setQ]=useState('Which team consumed the most tokens?'); const [ans,setAns]=useState(null); return <section className="panel ask"><div className="askGlow">AI</div><h2>Ask Bureau</h2><p>Ask about agents, LOBs, models, tools, tokens, cost and threats.</p><div className="askbar"><input value={q} onChange={e=>setQ(e.target.value)} /><button onClick={()=>post('/bureau/ask',{question:q}).then(setAns)}>Ask</button></div>{ans&&<div className="answer"><b>{ans.answer}</b>{ans.data&&ans.data.map((x,i)=><p key={i}>{x.agent_name}: {x.monthly_tokens} tokens · ${x.monthly_cost}</p>)}</div>}<div className="chips"><button onClick={()=>setQ('Which agents use Athena?')}>Which agents use Athena?</button><button onClick={()=>setQ('Show under review agents')}>Show threats</button><button onClick={()=>setQ('Top token cost agents')}>Top token cost agents</button></div></section> }
