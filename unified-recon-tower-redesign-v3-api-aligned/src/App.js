...
function getToolModel(response) {
  ...
  const rawResult = firstTool?.toolResponse?.result || {};
...
function calculateDelta(actual, baseline) {
  ...
  if (p90Match) {
    ...
    if (actualNumber > p90) return `+${formatNumber(actualNumber - p90)} vs p90`;
    if (p10Match) {
      const p10 = Number(p10Match[1].replace(/,/g, ''));
      if (actualNumber < p10) return `-${formatNumber(p10 - actualNumber)} vs p10`;
    }
    return 'Within band';
  }

  if (directBaseline !== null) {
    const delta = actualNumber - directBaseline;
...
function App() {
  ...
  useEffect(() => {
    const targets = document.querySelectorAll(
      '.answer-panel, .loading-card, .generic-response, .glass-card, .kpi-card, .landing-card, .lane-row, .attention-item, .timeline-item, .file-card, .driver-card, .anomaly-card, .action-item, .incident-item, .cost-recon-item, .cost-anomaly-item, .recon-cost-row, .cost-node, .generic-chart-row'
    );
...
function LoadingState() {
  return (
    <section className="loading-card">
      ...
      <small>Waiting for API response. The workspace will resolve when the backend returns.</small>
    </section>
  );
}
    ...
    <section className="workflow-grid operational-brief">
      ...
      {!!result?.charts?.length && (
        <div className="glass-card span-12">
          <SectionTitle eyebrow="Supporting data" title="API charts" />
          <GenericSeriesCharts charts={result.charts} />
        </div>
      )}
    </section>
  );
...
function InvestigationOverview({ result, stages }) {
  ...
  return (
    <div className="overview-grid">
      <div className="overview-panel"><p className="eyebrow">Current stage</p><h4>{getStageDisplayName({ name: result?.currentStage || result?.stage || 'Unavailable' })}</h4><p>{failedStage?.issue || 'No blocking stage issue was returned.'}</p></div>
      <div className="overview-panel"><p className="eyebrow">Operator readout</p><h4>{titleCase(result?.status || 'Unknown')}</h4><p>{metricValue(result?.name || result?.reconId)} is {normalizeStatus(result?.status || 'unknown')} with {metricValue(result?.progress, 0)}% of the journey completed.</p></div>
    </div>
  );
}
  ...
  return (
    <section className="workflow-grid sla-workspace">
      ...
      <div className="glass-card span-7 prediction-card">
        ...
        <GenericSeriesCharts charts={charts} compact />
      </div>
      ...
      <div className="span-7 anomaly-stack">
        {anomalies.map((anomaly, index) => (
          <div className={`glass-card anomaly-card severity-${String(anomaly.severity).toLowerCase()}`} key={`${anomaly.type}-${anomaly.stageType}-${index}`}>
            ...
            <div className="evidence-metrics"><MetricMini label="Stage" value={getStageDisplayName({ type: anomaly.stageType })} /><MetricMini label="Actual" value={anomaly.actual} /><MetricMini label="Baseline" value={anomaly.baseline} /><MetricMini label="Delta" value={anomaly.delta} /></div>
          </div>
        ))}
...
function GenericSeriesChart({ chart }) {
  ...
  const max = Math.max(...series.flatMap((item) => safeArray(item.data).map((value) => Number(value) || 0)), 1);
  return (
    <div className="generic-chart-block">
          ...
          <div className="generic-chart-row" key={`${label}-${rowIndex}`}>
            ...
            <div>
              {series.map((item) => {
                ...
                return <i key={item.name} style={{ '--bar': `${Math.max(2, (value / max) * 100)}%` }} title={`${item.name}: ${value}`}><em>{formatNumber(value)}</em></i>;
              })}
            </div>
          </div>
        ))}
...
function CostIntelligence({ result }) {
  ...
  return (
    <section className="workflow-grid cost-workspace">
      ...
      <div className="glass-card span-8"><SectionTitle eyebrow="Cost by stage" title="Where compute is being consumed" /><div className="cost-bars">{byStage.map((item, index) => <div className="cost-row" key={item.stage || index}><span>{getStageDisplayName({ type: item.stage })}</span><div><i style={{ '--bar': `${item.share || 0}%` }} /></div><strong>{money(currency, item.cost)}</strong></div>)}{!byStage.length && <EmptyState text="No stage-level cost returned." />}</div></div>
      <div className="glass-card span-12 cost-field-card"><SectionTitle eyebrow="All recons" title="Recon cost field" helper="Every node is one recon. Larger nodes are consuming more of today’s operational cost." /><CostField items={byRecon} currency={currency} /></div>
...
function CostField({ items, currency }) {
  ...
  return <div className="cost-field">{items.map((item, index) => { const cost = Number(item.cost) || 0; const ratio = cost / maxCost; const size = 54 + ratio * 72; return <div className="cost-node is-visible" key={item.reconId || item.name || index} style={{ '--size': `${size}px`, '--rise': `-${Math.round(ratio * 50)}px`, '--node-delay': `${index * 70}ms` }}><span>{item.reconId || item.name || `Recon ${index + 1}`}</span><strong>{money(currency, cost)}</strong><em>{item.reason || item.status || 'Cost driver unavailable'}</em></div>; })}</div>;
}
