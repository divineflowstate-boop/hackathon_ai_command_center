...
function App() {
  ...
  useEffect(() => {
    const targets = document.querySelectorAll(
      '.answer-panel, .loading-card, .generic-response, .glass-card, .kpi-card, .landing-card, .lane-row, .attention-item, .timeline-item, .file-card, .driver-card, .anomaly-card, .action-item, .incident-item, .cost-anomaly-item, .recon-cost-row, .cost-grid-card, .cost-stage-row, .generic-chart-row, .daily-chart-mini, .daily-mini-row, .thin-stage, .overview-panel, .metric-mini'
    );

    targets.forEach((element, index) => {
      element.classList.remove('is-visible');
      element.classList.add('reveal-ready');
      element.style.setProperty('--reveal-delay', `${Math.min(index * 34, 320)}ms`);
    });
    ...
    window.requestAnimationFrame(() => targets.forEach((element) => observer.observe(element)));
